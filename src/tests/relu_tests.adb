...
package body Relu_Tests is
   ...
   procedure Test_Forward_R (T : in out Test_Cases.Test_Case'Class) is
       --ReLU vars
        ...
        R_Test_Input : Tensor;
   begin
      R_Activated.Data := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((0.0, 0.0, 0.0, 0.0), (2, 2)));
      R_Activated.Grad := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((0.0, 0.0, 0.0, 0.0), (2, 2)));

      R_Test_Input.Data := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, -3.0, 4.0, -5.0), (2, 2)));
      R_Test_Input.Grad := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, -3.0, 4.0, -5.0), (2, 2)));

      declare
         --ReLU object
        R : aliased Mlengine.Operators.ReLU_T := (Activated => R_Activated);
        --expected answer
        Answer : ST_GPU.GPU_Tensor := ST.GPU.To_Tensor((1.0, 0.0, 4.0, 0.0), (2,2));
        --result
        Result : ST_GPU.GPU_Tensor := R.Forward(R_Test_Input);
      begin
   ...
   procedure Test_Backward_R (T : in out Test_Cases.Test_Case'Class) is
      --ReLU vars
        ...
        R_dY : Tensor;
   begin
      R_Activated.Data := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, 0.0, 4.0, 0.0), (2, 2)));
      R_Activated.Grad := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, 0.0, 4.0, 0.0), (2, 2)));

      R_dY.Data := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, 3.0, 4.0, 5.0), (2, 2)));
      R_dY.Grad := new ST_GPU.GPU_Tensor'(ST_GPU.To_Tensor ((1.0, 3.0, 4.0, 5.0), (2, 2)));

      declare
         L : aliased Mlengine.Operators.ReLU_T := (Activated => R_Activated);
         Answer : ST_GPU.GPU_Tensor := ST.GPU.To_Tensor((1.0, 0.0, 4.0, 0.0), (2,2));
         Result : ST_GPU.GPU_Tensor := L.Backward(R_dY);
      begin
