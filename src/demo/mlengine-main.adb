...
subtype Range_T is Positive range 1 .. 20;

Proba : Tensor;
--Target : Target_Array (Range_T) := (others => 0);
X: Tensor; --input
X_Target : Target_Array := (1,1,3,1,3,3,2,2,2,3,1,2,3,1,1,3,3,2,3,2);
--SLM : aliased Mlengine.LossFunctions.SoftLossMax_T (Range_T'Last);
...
Mean : Orka.Float_32 := Soft_Max_Loss (Z, Target);
 
begin
    --Put_Line (OT.Image (Test));
    Put_Line (Mean'Image);
    --SLM.Proba := Proba;
    --SLM.Target := Target;
    ...
    --SLM.Target := X_Target;
    --SLM.Proba.Data := new ST_CPU.CPU_Tensor'(ST_CPU.To_Tensor (      (0.909008014,0.00851531049,0.0824766752,
...
--                                                                     ), (20,3)));
    ...
    --  declare
    --      Fwd : Orka.Float_32 := Forward (SLM, X.Data.All, X_Target);
    --      Bck : ST_CPU.CPU_Tensor := Backward (SLM);
    --  begin
    --      Put_Line ("Forward: " & Fwd'Image);
    --      Put_Line ("Backward: " & ST_CPU.Image (Bck));
    --  end;
